const { Command, Store, Stopwatch } = require('@sakaruzero/klasa');
...
module.exports = class extends Command {
	constructor(...args) {
		super(...args, {
			...
			runIn: ['text', 'dm'],
			permissionLevel: 10,
			description: language => language.get('BOT_OWNER_RELOAD_DESCRIPTION'),
	...
	async run(message, [piece]) {
		...
		if (piece instanceof Store) {
			...
			return message.sendLocale('BOT_OWNER_RELOAD_ALL', [piece, timer.stop()]);
		}
		...
		try {
			...
			return message.sendLocale('BOT_OWNER_RELOAD', [itm.type, itm.name, timer.stop()]);
		} catch (err) {
			piece.store.set(piece);
			return message.sendLocale('BOT_OWNER_RELOAD_FAILED', [piece.type, piece.name]);
		}
	...
	async everything(message) {
		...
		return message.sendLocale('BOT_OWNER_RELOAD_EVERYTHING', [timer.stop()]);
	};
